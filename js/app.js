class App {
    ...
    _ensureDefaultMembers() {
        const seeded = localStorage.getItem('pbDefaultMembersSeeded') === 'true';
        if (!seeded) {
            for (let i = 0; i <= 8; i++) {
            ...
            localStorage.setItem('pbDefaultMembersSeeded', 'true');
        }
    }
    ...
    _renderVoters() {
        ...
        let html = '<div style="overflow-x:auto;"><table class="results-table"><tr><th>#</th><th>Name</th><th>Groups Rated</th><th>Members Rated</th><th>Status</th>' + (isTeacher ? '<th>Action</th>' : '') + '</tr>';
        allNames.forEach((name, i) => {
            ...
            const safeName = SafeHtml.escapeText(name);
            html += `<tr>
                <td>${i + 1}</td>
                ...
                ${isTeacher ? `<td><button class="btn btn-danger" style="padding:6px 12px;font-size:11px;" onclick="window.app._deleteVoterEvaluations('${safeName.replace(/'/g, "\\'")}')">Delete Vote</button></td>` : ''}
            </tr>`;
        });
        html += '</table></div>';
    ...
    async _deleteVoterEvaluations(voterName) {
        if (!voterName) return;
        if (!confirm(`Permanently delete ALL votes by "${voterName}"? This cannot be undone.`)) return;
        if (this._evaluationUnsubscribe) {
            this._evaluationUnsubscribe();
            this._evaluationUnsubscribe = null;
        }
        this._markResultsMutation();
        try {
            await this.storage.deleteVoterEvaluations(voterName);
        } catch (e) {}
        this.evaluations.deleteVoterEvaluations(voterName);
        this._renderVoters();
        if (this.tabManager && this.tabManager.activeTab === 'results') {
            this._refreshResults();
        } else {
            this._markResultsMutation();
        }
